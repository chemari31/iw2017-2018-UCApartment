...
public class MainScreen extends VerticalLayout implements ViewDisplay {
	...
	void init() {
		...
		FileResource frPolitica = new FileResource(new File("Politica de la empresa.pdf"));
		if(frPolitica != null) {
			FileDownloader fdPolitica = new FileDownloader(frPolitica);
			fdPolitica.extend(politica);
		}
	...
	{	
		try {
			...
			doc.close();
			//Notification.show("Política de empresa generada en el servidor.");
