...
@SpringView(name = editPerfilView.VIEW_NAME)
public class editPerfilView extends VerticalLayout implements View
{
	
	public static final String VIEW_NAME="editPerfilView";
	...
	@Autowired
	public editPerfilView(UsuarioService usuarioService) {
	...
	void init() {
		...
		nombreUsuario.setValue(user.getUsername());
		email.setValue(user.getEmail());
		dni.setValue(user.getDni());
        ...
        Button registro = new Button("Registro", evt -> {
        	if(binder.isValid()) { // Si todas las validaciones se han pasado
                ...
                user.setNombreUsuario(sNombreUsuario);
                user.setPassword(sPassword);
                user.setEmail(sEmail);
                user.setNombre(sNombre);
                user.setApellidos(sApellidos);
                user.setDni(sDNI);
                
                usuarioService.save(user);
                
                Notification.show("Su perfil se ha editado satisfactoriamente.", Notification.Type.HUMANIZED_MESSAGE );
                
                getUI().getNavigator().navigateTo(MiPerfilView.VIEW_NAME);
        ...
        Button cancelar = new Button("Cancelar", evt ->{
	...
	private boolean registro(Usuario usuario) {
		...
		if(!usuarioService.nombreUsuarioExistente(usuario.getNombreUsuario())
			...
			usuarioService.save(user); // Se guarda el usuario en la BD
